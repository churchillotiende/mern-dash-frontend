...
function SubscriptionHistoryList() {
  ...
  useEffect(() => {
    ...
    if (subscriptionInvoiceStatus === "idle") {
      store.dispatch(
        fetchSubscriptionInvoices({ accessToken: session.user.accessToken })
      );
    }
  }, [session, status, subscriptionInvoiceStatus]);
                ...
                <>
                  ...
                  <td className="py-0 pl-14 2xl:pl-4">
                    <span className="flex justify-end items-center w-full gap-2">
                      <Link href={getInvoiceUrl(item.id)}>
                        <a target="_blank">
