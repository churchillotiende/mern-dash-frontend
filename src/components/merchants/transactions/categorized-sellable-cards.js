...
export default function CategorizedSellableCards() {
  ...
  const debouncedSearchRequest = useMemo(
    () =>
      debounce(async (term) => {
        if (!term || !session || status !== "authenticated") {
          return;
        }
  ...
  }, [session, status, sellablesListStatus]);
  ...
  useEffect(() => {
    ...
    if (sellableCategoryStatus === "idle") {
      store.dispatch(fetchSellableCategories(params));
      // searchInputRef.current.select();
    }
  }, [session, status, sellableCategoryStatus]);
